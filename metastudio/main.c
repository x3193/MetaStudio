...
typedef struct
{
	char			name[32];
	int				parent;
	mat4_t			global;		// transform vector joint-space to global-space
	mat4_t			offset;		// transform vector global-space to joint-space
	vec3_t			bmin;
	vec3_t			bmax;
...
=================
*/
void Grab_Skeleton( s_bone_t *pbones )
{
	int		count, index;
	vec3_t	pos;
	vec4_t	rot;
			...
			for ( i = 0; i < 3; i++ )
			{
				...
				if ( fgets( line, sizeof( line ), input ) )
				{
								...
								&v.bone[3], &v.weight[3] ) ) >= 9 )
					{
						...
						pTri->indices[i] = lookup_vertex( pmodel, &v );
					}
					else
					{
						Error( "%s : error at line %d", filename, linecount );
	...
	while ( fgets( line, sizeof( line ), input ) )
	{
								...
								&rot[0], &rot[1], &rot[2], &rot[3] ) ) >= 7 )
		{
			if ( panim->node[iBone].parent == -1 )
			{
				...
				GL_MatrixQuaternion( mat2, rot );
				VectorCopy( mat2[3], pos );
			}
...
void Build_BoneTable( void )
{
	...
	for ( i = 0; i < pmodel->numbones; i++ )
	{
		...
		MatrixCopy( pmodel->skeleton[i].global, bonetable[numbones].global );
		MatrixCopy( pmodel->skeleton[i].offset, bonetable[numbones].offset );
		numbones++;
	}
}
		...
		for ( i = 0; i < nummodels; i++ )
		{
			...
			for ( j = 0; j < model[i].numverts; j++ )
			{
				...
				for ( k = 0; k < pv->numbones; k++ )
				{
					...
					GL_VectorTransform( pv->pos, bonetable[ pv->bone[k] ].offset, p );
...
void Build_SequenceBox( void )
{
	...
	for ( i = 0; i < numseq; i++ )
	{
		...
		for ( j = 0; j < sequence[i].numblends; j++ )
		{
			...
			{
				static mat4_t	bonetransform[ META_STUDIO_BONES ];	// transform position joint-space to global-space
				static mat4_t	bonetransform_skin[ META_STUDIO_BONES ];	// transform position bind-pose to current-pose in global-space
				...
				for ( n = 0; n < numbones; n++ )
				{
					...
					GL_QuaternionMatrix( panim->quat[n][k], bonematrix );
					VectorCopy( panim->pos[n][k], bonematrix[3] );

					parent = bonetable[n].parent;

					if ( parent == -1 )
					{
						MatrixCopy( bonematrix, bonetransform[n] );
						GL_ConcatTransforms( bonetransform[n], bonetable[n].offset, bonetransform_skin[n] );
					}
					else
					{
						GL_ConcatTransforms( bonetransform[parent], bonematrix, bonetransform[n] );
						GL_ConcatTransforms( bonetransform[n], bonetable[n].offset, bonetransform_skin[n] );
					}
				}
				...
				for ( n = 0; n < nummodels; n++ )
				{
					...
					for ( m = 0; m < pmodel->numverts; m++ )
					{
						...
						{
							GL_VectorTransform( pv->pos, bonetransform_skin[ pv->bone[w] ], tmp );
...
{
	static char	smdfilename[ META_STUDIO_BLENDS ][ 1024 ];
	int			depth;
	int			i;
	int			numblends;
...
void WriteBoneInfo( void )
{
	...
	for ( i = 0; i < numbones; i++ )
	{
		...
		MatrixCopy( bonetable[i].global, pbone[i].global );
		MatrixCopy( bonetable[i].offset, pbone[i].offset );
	}
...
void WriteModel( void )
{
	...
	for ( iModel = 0; iModel < nummodels; iModel++ )
	{
		...
		for ( iMesh = 0; iMesh < model[iModel].nummesh; iMesh++ )
		{
			...
			pmesh[iMesh].skinref = model[iModel].mesh[iMesh].skinref;

			for ( iTri = 0; iTri < pmesh[iMesh].numtris; iTri++ )
			{
				memcpy( ptris[iTri].indices, model[iModel].mesh[iMesh].tris[iTri].indices, sizeof( ptris[iTri].indices ) );
			}
